...
from .ui import print_plan_summary, print_plan_table, print_stats_table
from .utils import (
    Category,
    console,
    get_files,
...
def sync(
    ...
    # 4. 过滤掉无需操作的文件
    active_plan = [item for item in plan if item.is_active]
    ...
    with get_progress("[cyan]正在执行操作: ", len(active_plan)) as progress:
        ...
        for item in processor.execute_plan(active_plan):
            progress.advance(task_id)
            if item.error_msg:
                has_errors = True
                if "Target directory unavailable" in item.error_msg:
                    target_unavailable = True

    if target_unavailable:
        console.print(
            "\n[bold red]错误: 目标目录不可用或无写入权限 (可能外接硬盘未挂载)。同步已中止。[/bold red]"
        )
    elif has_errors:
        console.print(
            "\n[bold yellow]执行结束，但发生了一些错误。请查看日志。[/bold yellow]"
        )
    else:
...
def stats(
    ...
    stats_data = processor.get_stats(month)
