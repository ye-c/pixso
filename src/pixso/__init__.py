...
def process(
    ...
    delete_duplicates: bool = typer.Option(False, "--delete-duplicates", help="删除完全重复的源文件"),
):
    """处理并归档媒体文件"""
    ...
    for category in ["archive", "snapshot"]:
        ...
        for month_dir in cat_path.iterdir():
            ...
            for media_type in ["p", "v", "misc"]:
                ...
                if media_dir.exists():
                    count = sum(1 for _ in media_dir.iterdir() if _.is_file() and not _.name.startswith("."))
